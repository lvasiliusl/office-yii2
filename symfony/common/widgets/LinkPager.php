...
class LinkPager extends \yii\widgets\LinkPager
{
     ...
     */
    public $prevPageCssClass = 'pagination-previous';
    /**
    ...
    protected function renderPageButtons()
    {
        ...
        if ($this->prevPageLabel !== false) {
            ...
            $buttons[] = $this->renderPrevPageButton($this->prevPageLabel, $page, $this->prevPageCssClass, $currentPage <= 0, false);
        }
    ...
    protected function renderPageButton($label, $page, $class, $disabled, $active)
    {
        ...
        if ($active) {
            $link .= Html::tag('span', 'You\'re on page', ['class' => 'show-for-sr']);
            $link .= Html::tag('span', 'Page', ['class' => 'mobile-only']);
        ...
        if (!$active) {
            $link = Html::a($label, $this->pagination->createUrl($page), $linkOptions);
        }
    ...
    protected function renderPrevPageButton($label, $page, $class, $disabled, $active)
    {
        ...
        if ($disabled) {
            Html::addCssClass($options, $this->disabledPageCssClass);
            $tag = ArrayHelper::remove($this->disabledListItemSubTagOptions, 'tag', 'span');
            $content = Html::tag($tag, $label, ['class' => 'mobile-hide']);
            $content .= Html::tag($tag, 'Prev', ['class' => 'mobile-only']);
